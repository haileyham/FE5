...
function TripList() {
  let areas = list.map((item, index) => {
    return (
      <li
        key={index.no}
        //key={index} 불러온 데이터에 no 이나 id 같은 key처럼 보이는 식별자가 없을 때 임의로 만듦.
  ...
  //if문 대신 쉽게 3항연산자로
  //식별자로서 넣어야 할 키가 존재하지 않는다면 index로 집어넣기
  ...
  return <ul className="list-area">{areas}</ul>; //만약 return안에 하나만있으면 () 안써도됨 //className안에 저렇게 쓸수있는이유는 가능하기때문zz/카멜케이스는 속성만 해당
}
