class VendingMachineEvents {
  ...
  bindEvent() {
      ...
      if (inputCost) {
        ...
        if (inputCost <= myMoneyVal && inputCost > 0) {
          ...
          this.myMoney.textContent =
            new Intl.NumberFormat().format(myMoneyVal - inputCost) + "원"; //myMoneyVal은 25,000원 //format(number) 메소드는 숫자를 통화 형식에 맞게 변환, 각 나라의 화폐단위로 해주는 것 //쉼표 d찎히는 위치 자동 계산..
    ...
    /**
     * 4. 획득 버튼 기능
     *  1) 장바구니에 있는 음료수 목록이 획득한 목록으로 이동
    ...
    this.btnGet.addEventListener("click", () => {
      ...
      for (const itemStaged of itemStaged) {//두 가지를 비교
